...
// Screen B matches Size Up Step 2, plus "Select Unit Giving IRR" from Screen A units.
...
export function renderScreenB(state) {
  ...
  const selectedUnitIds = Array.isArray(incident.selectedUnitIds) ? incident.selectedUnitIds : [];
  ...
  // Optional: require IRR unit selection to proceed
  const canGoNext = !!(irr.irrUnitId && selectedUnitIds.includes(irr.irrUnitId));

  return `
    <section class="card">
      <div class="helper-text">Step 2 of 3</div>
      ...
      <div class="grid" style="margin-top:10px;">
        ${
          selectedUnits.length
            ? selectedUnits.map((u) => `
              <button type="button"
                class="choice irr-unit-choice ${irr.irrUnitId === u.id ? "selected" : ""}"
                data-irr-unit="${u.id}">
                ${u.label}
              </button>
            `).join("")
            : `<div class="helper-text">No units selected on Screen A. Go back and pick units.</div>`
        }
      </div>
    </section>
    ...
    <section class="card">
      ...
      <footer class="screen-footer">
        <button class="nav-btn nav-btn-secondary" id="irrBackBtn">◀ Back</button>
        <button class="nav-btn nav-btn-secondary" id="irrStartOverBtn">Start Over</button>
        <button class="nav-btn nav-btn-primary" id="toTacticalBtn" ${!canGoNext ? "disabled" : ""}>
          Next: Tactical View ▶
        </button>
      </footer>
    </section>
...
export function attachHandlersB() {
  // IRR unit selection (from Screen A selected units)
  document.querySelectorAll(".irr-unit-choice").forEach((btn) => {
    btn.addEventListener("click", () => {
      const unitId = btn.dataset.irrUnit;
      if (unitId) setIrrField("irrUnitId", unitId);
    });
  });
...
/* ---------- Text builder (now includes "From [Unit]" using irr.irrUnitId) ---------- */
...
function buildIrrText(state){
  ...
  const apparatus = irrUnit ? irrUnit.label : "";
