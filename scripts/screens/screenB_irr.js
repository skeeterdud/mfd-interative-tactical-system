...
// Screen B built to match your Size Up “Step 2” experience.
// FIX: Debounce text inputs so typing doesn't lose focus (no re-render per keystroke).
...
export function renderScreenB(state) {
  ...
  return `
    <section class="card">
      ...
      <p class="helper-text">Tap buttons to build the IRR. Type in the free-text fields as needed.</p>
    </section>
...
/** Debounce state updates for text fields so typing doesn't lose focus. */
function setIrrFieldDebounced(field, value, delay = 250) {
  if (typingTimers.has(field)) clearTimeout(typingTimers.get(field));
  typingTimers.set(
    field,
    setTimeout(() => {
...
export function attachHandlersB() {
  ...
  // Text inputs (DEBOUNCED + commit-on-blur)
  wireDebouncedInput("irrOccupancyOther", "occupancyOther");
  wireDebouncedInput("irrProblemLocationText", "problemLocationText");
  wireDebouncedInput("irrIapLocationOther", "iapLocationOther");
  wireDebouncedInput("irrCommandText", "commandText");
...
function wireDebouncedInput(elementId, stateField) {
  const el = document.getElementById(elementId);
  if (!el) return;

  // While typing: debounce
  el.addEventListener("input", () => {
    setIrrFieldDebounced(stateField, el.value, 250);
  });

  // When leaving the field: commit immediately
  el.addEventListener("blur", () => {
    setIrrField(stateField, el.value);
  });
}
...
/* -------------------- Text builder (matches Size Up logic) -------------------- */
...
function buildIrrText(state){
  ...
  const apparatus = ""; // left blank in tactical-system IRR for now
