...
// =========================
import { ALL_UNITS, setScreen, setIncidentField, toggleIncidentUnit, resetAll } from "../state.js";

export function renderScreenA(state){
  const battalion = Array.isArray(state.incident.battalion) ? state.incident.battalion : [];
  ...
  const unitLabelById = (id) => (ALL_UNITS.find(x => x.id === id)?.label || id);
    ...
    <section class="card">
      <div class="block ops">
        ...
        <div class="grid" style="grid-template-columns:repeat(2,minmax(0,1fr));">
          ${[
            ...
            <button type="button" class="choice bc-pill ${isBcSelected(b.code) ? "selected" : ""}" data-bc="${b.code}">
              ${b.label}
            </button>
          `).join("")}
        </div>
      ...
      <div class="block ops" style="margin-top:12px;">
        ...
        <footer class="screen-footer">
          <button class="nav-btn" id="incidentResetBtn">Start Over</button>
          <button class="nav-btn nav-btn-primary" id="toIrrBtn" ${!canGoNext ? "disabled" : ""}>Start IRR ▶</button>
        </footer>
      </div>
    </section>
  `;
...
export function attachHandlersA(state){
  document.querySelectorAll(".bc-pill").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (!bc) return;
      const cur = Array.isArray(state.incident.battalion) ? state.incident.battalion.slice() : [];
